            ...
            {groupedProducts.length > 0 ? groupedProducts.map(group => (
              <div key={group.name} className="space-y-4">
                <div className="flex items-center space-x-4">
                  ...
                  <h3 className="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-[0.3em] whitespace-nowrap">{group.name}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
              <div className="space-y-3">
                {sales.length > 0 ? sales.slice(0, 20).map((sale) => (
                  <div key={sale.id} className={`flex items-center justify-between p-5 rounded-[1.5rem] border transition-all ${sale.status === 'refunded' ? 'bg-rose-50/50 border-rose-100' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700'}`}>
                    <div>
                    ...
                    <div className="flex items-center space-x-6">
                      ...
                      {sale.status !== 'refunded' && <button onClick={() => handleRefund(sale)} className="bg-rose-600 text-white px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg">Rembourser</button>}
                    </div>
                  </div>
