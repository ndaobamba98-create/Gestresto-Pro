...
import { Search, Plus, Minus, Trash2, ShoppingBag, Utensils, Monitor, Banknote, ChevronLeft, Layers, MapPin, Coffee, Package, Truck, Download } from 'lucide-react';
...
const POS: React.FC<Props> = ({ products, onSaleComplete, config, session, onOpenSession, onCloseSession, notify }) => {
  ...
  const groupedProducts = useMemo(() => {
    ...
    const categoriesToDisplay = activeCategory === 'Tous' ? sortedCategories : sortedCategories.filter(c => c === activeCategory);
  ...
  const handleExportSessionReport = (closingBalance: number) => {
    ...
    const data = [{
      ...
      'Clôturée le': new Date().toLocaleString(),
      'Solde Initial': session.openingBalance,
      'Ventes Attendues': session.expectedBalance - session.openingBalance,
      'Solde Final Attendu': session.expectedBalance,
    ...
    XLSX.writeFile(workbook, `Rapport_Session_${session.cashierName}_${Date.now()}.xlsx`);
  };
    ...
    <div className="flex flex-col h-full gap-4 overflow-hidden">
      <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center space-x-3 overflow-x-auto scrollbar-hide shrink-0">
        ...
        <div className="flex items-center space-x-2">
          {allLocations.map((loc) => {
            ...
            return (
              <button key={loc.id} onClick={() => setActiveLocation(loc.id)} className={`relative px-4 py-3 rounded-2xl flex flex-col items-center min-w-[90px] transition-all border-2 ${activeLocation === loc.id ? 'bg-purple-600 border-purple-400 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-500 hover:bg-slate-100'}`}>
                <loc.icon size={16} className="mb-1" />
                <span className="text-[9px] font-black uppercase tracking-tight">{loc.label}</span>
                {hasItems && <span className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900">{pendingCarts[loc.id].length}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex-1 flex flex-col space-y-4 min-w-0">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            ...
            <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-hide">
              <button onClick={() => setActiveCategory('Tous')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeCategory === 'Tous' ? 'bg-purple-600 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 hover:bg-slate-100'}`}>Tous</button>
              {sortedCategories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeCategory === cat ? 'bg-purple-600 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 hover:bg-slate-100'}`}>{cat}</button>
              ))}
            </div>
          </div>
          <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide space-y-10">
            {groupedProducts.length > 0 ? groupedProducts.map(group => (
              <div key={group.name} className="space-y-4">
                <div className="flex items-center space-x-4">
                  <div className="p-1.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 rounded-lg"><Layers size={14} /></div>
            ...
            )) : <div className="h-full flex flex-col items-center justify-center opacity-30 text-center space-y-4 py-20"><Utensils size={48} /><p className="text-sm font-black uppercase tracking-widest">Aucun résultat trouvé</p></div>}
          </div>
        </div>
            ...
            {currentCart.map(item => (
              <div key={item.product.id} className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 animate-slideInRight group">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button onClick={() => removeFromCart(item.product.id)} className="ml-2 p-1 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={12} /></button>
                </div>
              </div>
            ))}
            {currentCart.length === 0 && <div className="h-full flex flex-col items-center justify-center opacity-20 text-center space-y-4 py-20"><Utensils size={48} /><p className="text-[10px] font-black uppercase tracking-widest">Le panier pour {activeLocation} est vide</p></div>}
          </div>
      ...
      {showClosingModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl border border-slate-200 dark:border-slate-800 animate-scaleIn">
            ...
            <div className="flex flex-col space-y-3">
              <button onClick={() => { handleExportSessionReport(closingInput); onCloseSession(closingInput); setShowClosingModal(false); }} className="w-full py-4 bg-rose-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all flex items-center justify-center">
                <Download size={16} className="mr-2" /> Valider & Exporter Rapport
              </button>
