...
import { 
  Plus, Search, Mail, Phone, Briefcase, Calendar, DollarSign, Users, Clock, FileText, UserCheck, LogIn, LogOut, Trash2, AlertTriangle, X, UserPlus, Edit3, Save, Download, Printer, QrCode, AlertCircle, Calculator, FileSpreadsheet, Paperclip, File
} from 'lucide-react';
...
const HR: React.FC<Props> = ({ employees, onUpdate, attendance, onUpdateAttendance, config, notify, userPermissions }) => {
  ...
  const handleClockIn = (emp: Employee) => {
    ...
    const newRecord: AttendanceRecord = { id: `ATT-${Date.now()}`, employeeId: emp.id, employeeName: emp.name, checkIn: timeStr, date: dateStr };
  ...
  const handleClockOut = (emp: Employee) => {
    const now = new Date();
    const timeStr = now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
    const updatedAttendance = attendance.map(rec => (rec.employeeId === emp.id && !rec.checkOut) ? { ...rec, checkOut: timeStr } : rec);
              ...
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <div className="w-full space-y-2 mb-6 text-left bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl">
                  ...
                  {emp.attachments && emp.attachments.length > 0 && (
                    <div className="flex items-center text-[10px] text-purple-600 font-black uppercase"><Paperclip size={10} className="mr-2" /> {emp.attachments.length} Documents</div>
                  )}
                </div>
                <div className="w-full grid grid-cols-1 gap-2">
                  <button onClick={() => setSelectedForPayslip(emp)} className="w-full bg-slate-900 text-white py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center hover:bg-purple-600 transition-all shadow-sm"><FileText size={14} className="mr-2" /> Fiche de Paie</button>
