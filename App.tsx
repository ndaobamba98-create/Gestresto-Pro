...
import { 
  LayoutDashboard, ShoppingCart, Package, BarChart3, Monitor, Settings as SettingsIcon, Sun, Moon, IdCard, LogOut, Clock as ClockIcon, FileText, Search, ArrowRight, User as UserIcon, Users, ChevronLeft, ChevronRight, UserPlus, LogIn, Key, ShieldCheck, ChevronDown, ArrowRightLeft, Bell, X, Check, Trash2, BellOff, Info, AlertTriangle, CheckCircle
} from 'lucide-react';
import { ViewType, Product, SaleOrder, Employee, ERPConfig, AttendanceRecord, User, CashSession, Expense, Customer, UserRole, AppNotification } from './types';
import { INITIAL_PRODUCTS, INITIAL_EMPLOYEES, INITIAL_CONFIG, APP_USERS, INITIAL_CUSTOMERS } from './constants';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!isLocked && currentUser?.role === 'admin') {
      ...
      if (lowStock.length > 0) {
        const notifId = 'low-stock-' + new Date().toISOString().split('T')[0];
        if (!notifications.find(n => n.id === notifId)) {
          addNotification(
            "Alerte Stocks", 
            `${lowStock.length} articles sont en rupture ou presque épuisés.`, 
            'warning'
          );
        }
      }
    }
  }, [isLocked, products, currentUser]);
  ...
  const currentSession = useMemo(() => sessionHistory.find(s => s.status === 'open' && (currentUser?.role === 'admin' ? true : s.cashierId === currentUser?.id)), [sessionHistory, currentUser]);

  const notifyUser = useCallback((title: string, message: string, type: 'success' | 'info' | 'warning' = 'info') => {
    console.log(`[NOTIFY] ${title}: ${message}`);
    // Auto-add to internal notifications if it's important
    if (currentUser?.role === 'admin') {
      addNotification(title, message, type === 'warning' ? 'warning' : 'info');
    }
  }, [currentUser]);
            ...
            {authMode === 'login' ? (
              <form onSubmit={handleLoginSubmit} className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2 relative">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 flex items-center">
                       <Users size={12} className="mr-2" /> IDENTIFIANT
                    </label>
                    <button
                      type="button"
                      onClick={() => setIsUserListOpen(!isUserListOpen)}
                      className="w-full bg-black/40 border-2 border-white/5 hover:border-purple-500 rounded-2xl py-4 px-6 text-white flex items-center justify-between transition-all group"
                    >
                      {selectedUser ? (
                        <div className="flex items-center space-x-3">
                          <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${selectedUser.color} flex items-center justify-center text-white font-black text-[10px]`}>
                        ...
                        <div className="max-h-60 overflow-y-auto scrollbar-hide py-2">
                          {allUsers.map(user => (
                            <button
                              ...
                              className="w-full p-4 hover:bg-white/5 flex items-center justify-between text-left transition-all group"
                            >
                              <div className="flex items-center space-x-4">
                                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${user.color} flex items-center justify-center text-white font-black text-xs`}>
                                  {user.initials}
                                </div>
                                <div>
                                  <p className="font-black text-white text-[11px] uppercase">{user.name}</p>
                  ...
                  <div className={`space-y-2 transition-all duration-500 ${selectedUserId ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 flex items-center">
                       <Key size={12} className="mr-2" /> MOT DE PASSE
                    </label>
                    <input 
                      type="password" 
                      value={passwordInput} 
                      onChange={e => setPasswordInput(e.target.value)} 
                      className="w-full bg-black/40 border-2 border-transparent focus:border-purple-500 rounded-2xl py-4 px-6 text-white font-black tracking-[0.5em] outline-none transition-all text-center" 
                      placeholder="••••" 
                    />
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-5 bg-purple-600 text-white rounded-3xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-purple-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center justify-center space-x-3"
                >
  ...
  const renderContent = () => {
    const commonProps = { 
      notify: notifyUser, 
      userPermissions: ['dashboard', 'pos', 'inventory', 'sales', 'invoicing', 'reports', 'hr', 'attendances', 'settings', 'manage_inventory', 'manage_session_closing'] as ViewType[], 
      t, 
    ...
    switch (activeView) {
      ...
      case 'pos': return <POS products={products} customers={customers} onUpdateCustomers={setCustomers} sales={sales} onSaleComplete={s => { setSales([s as SaleOrder, ...sales]); if(currentUser?.role === 'admin') addNotification("Vente Validée", `Nouvelle commande de ${(s as SaleOrder).total} ${config.currency}`, 'success'); }} onRefundSale={() => {}} config={config} session={currentSession || null} onOpenSession={(bal, cid) => setSessionHistory([{id:`S-${Date.now()}`, openedAt: new Date().toISOString(), openingBalance: bal, expectedBalance: bal, totalCashSales: 0, status: 'open', cashierName: currentUser!.name, cashierId: cid} as CashSession, ...sessionHistory])} onCloseSession={bal => setSessionHistory(sessionHistory.map(s => s.id === currentSession?.id ? {...s, status: 'closed', closingBalance: bal} as CashSession : s))} userRole={currentUser!.role} userPermissions={commonProps.userPermissions} {...commonProps} />;
      ...
      case 'settings': return <Settings products={products} onUpdateProducts={setProducts} config={config} onUpdateConfig={setConfig} rolePermissions={[]} onUpdatePermissions={() => {}} currentUser={currentUser!} allUsers={allUsers} onUpdateUsers={setAllUsers} userPermissions={commonProps.userPermissions} t={t} notify={notifyUser} />;
    ...
    <div className={`flex h-screen overflow-hidden theme-${config.theme} ${darkMode ? 'dark text-slate-100' : 'text-slate-900'}`}>
      ...
      <aside className={`${isSidebarOpen ? 'w-72' : 'w-24'} bg-slate-900 transition-all duration-500 ease-in-out flex flex-col z-20 shadow-2xl relative`}>
        ...
        <button 
          ...
          className="absolute -right-3 top-32 w-7 h-7 bg-purple-600 text-white rounded-full flex items-center justify-center shadow-xl z-30 hover:scale-110 active:scale-90 transition-all border-4 border-slate-950"
        >
        ...
        <nav className="flex-1 mt-6 space-y-1 px-3 overflow-y-auto scrollbar-hide">
          {[
            ...
            <button 
              ...
              className={`w-full flex items-center p-4 rounded-2xl transition-all ${activeView === item.id ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950 relative">
        <header className="h-24 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 z-10">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={() => setDarkMode(!darkMode)} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl transition-colors text-slate-600 dark:text-slate-400 hover:text-purple-600">
            ...
            <div className="relative">
               <button 
                 ...
                 className={`p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm relative transition-all hover:scale-105 active:scale-95 ${unreadCount > 0 ? 'animate-pulse' : ''}`}
               >
                 <Bell size={20} className={unreadCount > 0 ? 'text-rose-500 fill-rose-500/10' : 'text-slate-500'} />
                 {unreadCount > 0 && (
                   <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-600 text-white rounded-full text-[10px] font-black flex items-center justify-center border-2 border-white dark:border-slate-900 animate-bounce">
                     {unreadCount}
                   </span>
                 )}
               </button>
               ...
               {isNotifOpen && (
                 <div className="absolute top-full right-0 mt-4 w-96 bg-white/95 dark:bg-slate-900/95 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl border-2 border-slate-100 dark:border-slate-800 z-[100] animate-scaleIn origin-top-right overflow-hidden">
                    <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                       <h3 className="text-xs font-black uppercase tracking-widest text-slate-900 dark:text-white">Alertes & Notifications</h3>
                       <div className="flex items-center space-x-2">
                          <button onClick={markAllAsRead} className="p-2 text-slate-400 hover:text-emerald-500 transition-all" title="Tout marquer comme lu">
                            <CheckCircle size={16} />
                          </button>
                          <button onClick={() => setNotifications([])} className="p-2 text-slate-400 hover:text-rose-500 transition-all" title="Vider">
                            <Trash2 size={16} />
                          </button>
                       </div>
                    </div>
                    <div className="max-h-[450px] overflow-y-auto scrollbar-hide py-2">
                       {notifications.length === 0 ? (
                         <div className="py-20 text-center space-y-3">
                            <BellOff size={40} className="mx-auto text-slate-300 dark:text-slate-700" />
                            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Aucune notification</p>
                         </div>
                       ) : (
                         notifications.map(notif => (
                           <div 
                             key={notif.id} 
                             className={`p-6 border-b border-slate-50 dark:border-slate-800 flex items-start space-x-4 transition-all hover:bg-slate-50/50 dark:hover:bg-slate-800/30 group ${!notif.read ? 'bg-purple-50/20 dark:bg-purple-900/5' : ''}`}
                           >
                              <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${notif.type === 'warning' ? 'bg-rose-100 text-rose-500' : notif.type === 'success' ? 'bg-emerald-100 text-emerald-500' : 'bg-blue-100 text-blue-500'}`}>
                                 {notif.type === 'warning' ? <AlertTriangle size={18} /> : notif.type === 'success' ? <CheckCircle size={18} /> : <Info size={18} />}
                              </div>
                              <div className="flex-1 min-w-0">
                                 <div className="flex justify-between items-start">
                                    <h4 className={`text-xs font-black uppercase tracking-tight truncate ${notif.read ? 'text-slate-500' : 'text-slate-900 dark:text-white'}`}>{notif.title}</h4>
                                    <span className="text-[8px] font-bold text-slate-400 uppercase whitespace-nowrap ml-2">{new Date(notif.timestamp).toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'})}</span>
                                 </div>
                                 <p className={`text-[10px] font-medium leading-relaxed mt-1 ${notif.read ? 'text-slate-400' : 'text-slate-600 dark:text-slate-300'}`}>{notif.message}</p>
                                 <div className="flex items-center space-x-4 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {!notif.read && (
                                      <button onClick={() => markNotificationAsRead(notif.id)} className="text-[8px] font-black uppercase text-emerald-600 flex items-center"><Check size={12} className="mr-1"/> Marquer lu</button>
                                    )}
                                    <button onClick={() => deleteNotification(notif.id)} className="text-[8px] font-black uppercase text-rose-500 flex items-center"><X size={12} className="mr-1"/> Supprimer</button>
                                 </div>
                              </div>
                           </div>
                         ))
            ...
            <div className="flex items-center space-x-1">
              <div className="flex items-center space-x-3 bg-white dark:bg-slate-800 p-2 pr-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                 ...
                 <div className="flex flex-col">
                    <span className="text-xs font-black dark:text-white leading-tight">{currentUser?.name}</span>
                    <span className="text-[8px] font-bold text-purple-600 uppercase tracking-widest">{currentUser?.role}</span>
                 </div>
              </div>
              
              <button 
                onClick={logoutAction} 
                className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-400 hover:text-rose-500 hover:border-rose-200 transition-all shadow-sm flex items-center justify-center group"
                title="Changer d'IDENTIFIANT / Quitter la session"
              >
